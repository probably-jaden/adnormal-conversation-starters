...
as_tibble(as_draws_df(bayes_lm))  %>% 
  dplyr::select(b_qAI_morals:b_qwar_vs_disease) %>% 
  rename_with(~ c("AI_morals", "Bad_hot_actors",  "Dreams", "Emotional_iq",  "Future_charity",  "Guys_intimidated",  "Human_takeover", 
                  "Insurance", "Korean_bias", "Laughter",  "Nurture_vs_nature", "Peacocks",  "Pools",
                   "Poor_entrepreneurs", "War_vs_disease"), 1:15) %>% 
  gather() %>% 
